...
public class CRCUtil {
    static int[] crc32_tab = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f, 0xe963a535,
	...
	public static long getChecksum(ByteBuffer buf) {
		int length = buf.position() + buf.remaining();
		return getChecksum(buf, 0, length);
	}

	public static long getChecksum(ByteBuffer buf, int off, int len) {
		int value = 0;

		int last = Math.min(off + len, buf.position() + buf.remaining());

		for (int i = off; i < last; i++) {
			value = crc32_tab[(value ^ buf.get(i)) & 0xff] ^ (value >>> 8);
		}
