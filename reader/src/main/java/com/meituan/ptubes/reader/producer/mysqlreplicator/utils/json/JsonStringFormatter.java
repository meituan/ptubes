...
/**
 * A {@link JsonFormatter} implementation that creates a JSON string
 * representation.
 */
public class JsonStringFormatter implements JsonFormatter {

	/**
	 * Value used for lookup tables to indicate that matching characters do not
	 * need to be escaped.
	 */
	private static final int ESCAPE_NONE = 0;

	/**
	 * Value used for lookup tables to indicate that matching characters are to
	 * be escaped using standard escaping; for JSON this means (for example)
	 * using "backslash - u" escape method.
	 */
	private static final int ESCAPE_GENERIC = -1;

	/**
	 * A lookup table that determines which of the first 128 Unicode code points
	 * (single-byte UTF-8 characters) must be escaped. A value of '0' means no
	 * escaping is required; positive values must be escaped with a preceding
	 * backslash; and negative values that generic escaping (e.g.,
	 * {@code \\uXXXX}).
	 */
	...
	public void value(double value) {
		// Double's toString method will result in scientific notation and loss
		// of precision
	...
	public void value(BigInteger value) {
		// Using the BigInteger.toString() method will result in scientific
		// notation, so instead ...
	...
	/**
	 * Append a generic Unicode escape (e.g., {@code \\uXXXX}) for given
	 * character.
	 *
	 * @param charToEscape
