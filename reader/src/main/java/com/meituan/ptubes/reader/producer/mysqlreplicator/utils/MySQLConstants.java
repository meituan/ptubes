...
public final class MySQLConstants {
	...
	// sql/log_event.h {@link https://dev.mysql.com/doc/internals/en/event-classes-and-types.html}
	public static final int UNKNOWN_EVENT = 0;
	public static final int START_EVENT_V3 = 1;
	public static final int QUERY_EVENT = 2; // Record DDL or BEGIN in row mode
	public static final int STOP_EVENT = 3; // Triggered when mysql closes or reset slave, etc.
	public static final int ROTATE_EVENT = 4; // The last event in each binlog connects to the next binlog file (the fileName of the next file in the data)
	public static final int INTVAR_EVENT = 5; // invalid in auto_increment related row mode
	...
	public static final int RAND_EVENT = 13; // Rand() function triggers invalid in row mode
	public static final int USER_VAR_EVENT = 14; // invalid in row mode related to user variables
	public static final int FORMAT_DESCRIPTION_EVENT = 15; // The first event of the binlog file, recording metadata information such as version number
	public static final int XID_EVENT = 16; // The two-phase commit xid record, marking the end of the (XA) transaction or COMMIT
	public static final int BEGIN_LOAD_QUERY_EVENT = 17;
	public static final int EXECUTE_LOAD_QUERY_EVENT = 18;
	public static final int TABLE_MAP_EVENT = 19; // Record table source data (library name, table name, number of columns, type of each column, etc.) in row mode, and provide rule reference for reading row records, followed by (multiple rows) WRITE /UPDATE/DELETE _ROWS_EVENT (usually in the scenario of adding, deleting or modifying multiple records in one SQL)
	public static final int PRE_GA_WRITE_ROWS_EVENT = 20; // deprecated, renamed to 23
	public static final int PRE_GA_UPDATE_ROWS_EVENT = 21; // deprecated, renamed to 24
	public static final int PRE_GA_DELETE_ROWS_EVENT = 22; // deprecated, renamed to 25
	public static final int WRITE_ROWS_EVENT = 23; // record the corresponding row data change record in row mode
	public static final int UPDATE_ROWS_EVENT = 24; // record the corresponding row data change record in row mode
	public static final int DELETE_ROWS_EVENT = 25; // record the corresponding row data change record in row mode
	public static final int INCIDENT_EVENT = 26; // triggered when master and slave are inconsistent
	public static final int HEARTBEAT_LOG_EVENT = 27; // master->slave makes the slave library know that the master library is still alive According to the documentation, there will be no such event in binlog "Events of this type do not appear in the binary or relay logs" . They are generated on a master server by the thread that dumps events and sent straight to the slave without ever being written to the binary log. The slave examines the event contents and then discards it without writing it to the relay log."
	...
	public static final int GTID_LOG_EVENT = 33; // Record GTID transaction number for GTID-based synchronization after version 5.6
	public static final int ANONYMOUS_GTID_LOG_EVENT = 34;
	public static final int PREVIOUS_GTIDS_LOG_EVENT = 35; // The gtid collection executed at the end of the last binlog file
}
